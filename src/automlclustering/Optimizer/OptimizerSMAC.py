...
from automlclustering.ClusteringCS.ClusteringCS import CONFIG_SPACE_MAPPING, build_kmeans_space, \
    build_paramter_space_per_algorithm, \
...
def preprocess_data(dataset, scaler=MinMaxScaler):
    """
...
class OptimizerHistoryEntry:
    ...
    def __init__(self, runtime, score, labels, config, iteration, score_name='ARI', additional_metrics=None):
    ...
    def to_dict(self, skip_labels=False):
        if skip_labels:
            return dict({'runtime': self.runtime, self.score_name: self.score, 'iteration': self.iteration,
    ...
    def __str__(self):
        return f"OptimizerHistoryEntry(runtime={self.runtime}, " \
               f"score={self.score}, labels={len(self.labels)}, " \
               f"config={self.configuration}), iteration={self.iteration}"
...
class AbstractOptimizer(ABC):
    ...
    def __init__(self, dataset, cvi: Union[CVIHandler.CVI] = CVICollection.CALINSKI_HARABASZ,
                 cs: ConfigurationSpace = None, n_loops=None, output_dir=None,
                 cut_off_time=30 * 60,
        ...
        if not cs:
            ...
            if isinstance(cs, str) and cs in CONFIG_SPACE_MAPPING.keys():
...
class SMACOptimizer(AbstractOptimizer):
    ...
    def __init__(self, dataset,
                 wallclock_limit: int = 360 * 60,
                 # wallclock_limit: int = 1,
                 cvi: CVI = None,
                 n_loops: int = None,
                 smac: type[SMAC4AC] = SMAC4HPO,
                 cs: ConfigurationSpace = None,
                 output_dir: str = None,
                 # cutoff_time: int = 1,
                 cutoff_time: int = 15 * 60,
                 true_labels=None,
                 optimization_function=smac_function,
                 seed=2,
                 limit_resources=True):
        super().__init__(dataset=dataset, cvi=cvi, cs=cs, n_loops=n_loops, output_dir=output_dir,
                         true_labels=true_labels, wallclock_limit=wallclock_limit, cut_off_time=cutoff_time,
                         seed=seed)
    ...
    def optimize(self, initial_configs=None) -> Union[SMAC4HPO, SMAC4AC]:
        ...
        :return:
        """
        tae_algorithm = partial(self.optimization_function, optimizer_instance=self)

        # Scenario object
        scenario = Scenario({"abort_on_first_run_crash": False,  # Do not crash if first run fails!
                             "run_obj": "quality",  # we optimize quality (alternatively runtime)
                             "runcount-limit": self.n_loops,  # max. number of function evaluations;
                             "cs": self.cs,  # configuration space
                             "deterministic": True,
                             "output_dir": self.output_dir,
                             "cutoff_time": 0.01,#self.cutoff_time,
                             # max duration to run the optimization (in seconds)
                             "wallclock-limit": self.wallclock_limit,
                             "seed": self.seed,
                             "memory_limit": 52000,  # In MB --> max. 32 GB + 20GB Swap
        ...
        # self.smac.solver.scenario.acq_opt_challengers = 10
        self.smac.optimize()
    ...
    def parse_result(self):
        ...
        with open('{}/runhistory.json'.format(out_dir)) as json_file:
            ...
            for iteration, runs in enumerate(data):
                ...
                entry = OptimizerHistoryEntry(labels=y_pred, runtime=optimizer_time, score=score,
                                              ...
                                              score_name=self.cvi.get_abbrev(), iteration=iteration + 1)
...
if __name__ == '__main__':
    cs = build_config_space([DBSCAN_ALGORITHM])
    X, y = make_moons(n_samples=1000000)
