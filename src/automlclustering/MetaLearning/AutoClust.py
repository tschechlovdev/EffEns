...
from MetaLearning import DataGeneration, MetaFeatureExtractor

# define random seed
from MetaLearning.MetaFeatureExtractor import extract_all_datasets, load_kdtree, query_kdtree
from Metrics.MetricHandler import MetricCollection, MLPMetric
...
def rw_run_offline_phase():
    ...
    for algo in clustering_algos:
        for X, y, dataset_name in zip(datasets, true_labels, d_names):
            ...
            opt_instance = SMACOptimizer(dataset=X, true_labels=y, metric=MetricCollection.ADJUSTED_RAND,
...
def rw_run_online_phase():
    ...
    for i in range(len(datasets)):
        ...
        t0 = time.time()
        names, meta_features = MetaFeatureExtractor.extract_landmarking(dataset, mf_set="meanshift")
        ...
        # assignm algorithm column
        rw_opt_result_for_dataset["algorithm"] = rw_opt_result_for_dataset.apply(lambda x: ast.literal_eval(x["config"])["algorithm"],
                                                           axis="columns")
        ...
        mlp_metric = MLPMetric(mlp_model=mlp)
        ...
        related_work_online_result_df['ARI'] = [MetricCollection.ADJUSTED_RAND.score_metric(data=None, labels=labels,
        ...
        if len(iterations > 0):
            ...
            for i in range(1, 101):
                if i in iterations:
                    ...
                    it_filtered = related_work_online_result_df[related_work_online_result_df["iteration"] == last_iteration]
        ...
        related_work_online_result_df = related_work_online_result_df.drop(
            [mlp_metric.get_abbrev(), 'labels', 'budget'],
            axis=1)
        ...
        related_work_online_result_all_datasets.to_csv(related_work_path / 'related_work_online_result.csv')
...
# train model
# todo: if this is too good, we should train the mlp for every dataset again...
...
rw_run_online_phase()
related_work_online_result_all_datasets = pd.read_csv(related_work_path / 'related_work_online_result.csv', index_col=None)