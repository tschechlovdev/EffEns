...
def extract_landmarking(X, mf_set):
    ...
    print(f"Executing {algo_instance.__class__.__name__}")
    start = time.time()
    labels = algo_instance.fit_predict(X)
    print(f"Finished - Took {time.time() - start}s")
    print(f"Finished executing {algo_instance.__class__.__name__}, got {len(np.unique(labels))} cluster labels")
...
def extract_all_datasets(datasets, path=Path(''), mf_Set='statistical', d_names=[], save_metafeatures=True):
    ...
    mf_result_file = path / f'{mf_set_to_string(mf_Set)}_metafeatures.csv'
    if mf_result_file.is_file():
        df = pd.read_csv(mf_result_file, index_col=None)
    else:
        df = pd.DataFrame()
    print(df)
    ...
    default_mfe_mfs = df.iloc[:, :-1].to_numpy()
...
def load_kdtree(path =Path(""), mf_set='statistical'):
    try:
        ...
        print(e)
        print(f"Generating kd_tree again from meta-features")
        mfs = pd.read_csv(path / (mf_set_to_string(mf_set) + "_metafeatures.csv"), index_col=0)
