...
def permutation_bipartite_matching(association_matrix_0, association_matrix_1, return_permutation_matrix=False):
    ...
    ref_indices, U_indices = max_bipartite_matching_ba(association_matrix_0, association_matrix_1)
    ...
    W = csr_matrix((np.repeat(1, ref_indices.size), (U_indices, ref_indices)), shape=(k_U, k_ref))
    return W
...
def min_k_partition_indices(ens: np.ndarray, k: int) -> np.ndarray:
    ...
    #return np.array([len(np.unique(x)) >= k for x in ens.transpose()]).nonzero()[0]
    # Moritz approach
