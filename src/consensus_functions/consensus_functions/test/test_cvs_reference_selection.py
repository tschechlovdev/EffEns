...
class CVSReferenceSelectionTest(unittest.TestCase):
    ...
    def test_eligible_partition_indices(self):
        ...
        min_k_indices = [(2, mink_2_indices), (4, mink_4_indices), (10, mink_10_indices), (11, min_k_11_indices)]
        for (mink, correct_indices) in min_k_indices:
            assert_array_equal(correct_indices, min_k_partition_indices(ens, mink))
    ...
    def test_reference_selection(self):
        ...
        mink_4_indices = np.array([0, 2, 3])
        mink_10_indices = np.array([3])
        legal_min_k_indices = [(2, mink_2_indices, 4), (4, mink_4_indices, 2), (10, mink_10_indices, 3)]
        for (min_k, mink_indices, highest_entropy_idx) in legal_min_k_indices:
            assert_array_equal(highest_entropy_idx, ada_select_reference(ens, min_k, made_up_decreasing_entropy_indices))
        # There are no partitions with at least 11 clusters, so no reference can be selected
        self.assertRaises(ValueError, ada_select_reference, ens, 11, made_up_decreasing_entropy_indices)
