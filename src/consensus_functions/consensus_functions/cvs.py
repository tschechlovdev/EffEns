...
from consensus_functions.datastructures.datastructures import binary_association_matrix
from consensus_functions.utils.utils import min_k_partition_indices, normalize_columns, partition_entropies, \
...
def ada_vote(Y: ndarray, k: int, mode: VoteMode) -> ndarray:
    ...
    raise ValueError("k_aggregated (" + str(k_aggregated) + ") < k (" + str(k) + "). This should never happen.")


def ada_vote_aggregate(Y, reference_k_min: int, mode: VoteMode = VoteMode.CVOTE, return_mse=False, 
    ...
    reference_index = ada_select_reference(Y, reference_k_min, decreasing_entropy_indices)
    ...
    return vote_aggregate(Y, mode, reference_index, decreasing_entropy_indices, return_mse, return_intermediate)


def ada_select_reference(Y: np.ndarray, reference_k_min: int, decreasing_entropy_indices: np.ndarray) -> int:
    """
    ...
    if(eligible_partition_indices.size == 0):
        raise ValueError("You called ada_vote with k=" + str(reference_k_min) + ".\nBut \
            there is no clustering with at least " + str(reference_k_min) + " clusters in the \
            provided ensemble.")
...
def vote_aggregate(Y: ndarray, mode: VoteMode, reference_index: int = None, aggregation_order=None, return_mse=False, 
    ...
    if (reference_index is None):
        ...
        raise ValueError("The provided reference index: " + str(reference_index) + "is out of bounds")
    ...
    for (i, partition_index) in enumerate(aggregation_order):
        ...
        U_current = binary_association_matrix(
            Y[:, partition_index].reshape(N, 1))
