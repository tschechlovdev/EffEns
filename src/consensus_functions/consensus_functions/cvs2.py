...
from consensus_functions.utils.utils import normalize_columns, partition_entropies
...
def ada_cvote_aggregate_simplified(Y: ndarray) -> ndarray:
    ...
    moving_reference_partition = binary_association_matrix(Y[:, max_entropy_index].reshape(N, 1)).toarray()
    for (count, j) in enumerate(decreasing_entropy_indices[1:]):
        current_partition = binary_association_matrix(Y[:, j].reshape(N, 1)).toarray()
        current_partition_normalized = normalize_columns(current_partition)
        current_projection = current_partition_normalized.T.dot(moving_reference_partition)
        ...
        fraction_current, fraction_reference = (1 / (count + 2)), ((count + 1) / (count + 2))
