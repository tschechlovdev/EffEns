...
def mm(Y: ndarray, k_out: int, iterations=50, attempts=1) -> ndarray:
    """
    ...
    if (H > 300):
        raise ValueError("Running MM with more than 300 partitions in the ensemble will\
...
def MM_single(Y: ndarray, k_out: int, iterations: int = 100):
    ...
    for it in range(iterations):
        ...
        for m in range(M):
            Ez[:, m] = alpha[m] * select_outcome_probabilities(outcome_probabilities[m], Y).prod(axis=1)
        ...
        if 0 in rowsums:
            raise ValueError("The number of partitions (H=" + str(H) + ") is too large,\
        ...
        for k in range(k_in_max):
            for j in range(H):
                outcome_probabilities[:, j, k] = Ez[points_for_partitions[k, j]].sum(axis=0)
        for m in range(M):
    ...
    return assignment, (Ez, alpha, outcome_probabilities), parameters, likelihoods[:it + 1], it


def copy_parameters(alpha: ndarray, outcome_probabilities: ndarray, Ez: ndarray) -> Tuple:
    """
...
def init_outcome_probabilities(M: int, H: int, Y: ndarray, Ez: ndarray, k_in_max: int):
    """
    ...
    for k in range(k_in_max):
        ...
        for j in range(H):
            outcome_probabilities[:, j, k] = Ez[points_for_partitions[j]].sum(axis=0)
    for m in range(M):
        outcome_probabilities[m] = outcome_probabilities[m] / Ez[:, m].sum()
    return outcome_probabilities


def MM_iterative(Y: ndarray, cluster_counts: ndarray, k_out: int, init: Tuple, iterations=100):
    """
