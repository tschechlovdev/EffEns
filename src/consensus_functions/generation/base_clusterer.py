...
def single_linkage(instances, k):
    return AgglomerativeClustering(linkage="single", n_clusters=k).fit_predict(instances)


def average_linkage(instances, k):
    return AgglomerativeClustering(linkage="average", n_clusters=k).fit_predict(instances)


def complete_linkage(instances, k):
    return AgglomerativeClustering(linkage="complete", n_clusters=k).fit_predict(instances)
...
clustering_functions: List[Callable] = [sklearn_default_kmeans, single_kmeans, single_linkage, 
                                        complete_linkage, average_linkage, optics_default]
