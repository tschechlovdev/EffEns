...
from consensus_functions.generation.base_clusterer import single_kmeans
from typing import Callable


def fixed_k_ensemble(instances: ndarray, ensemble_k: int, nr_partitions: int,
...
def k_range_ensemble(instances: ndarray, ensemble_k_range,
    ...
    ens = np.empty((instances.shape[0], nr_partitions), dtype=int)
    for j in range(nr_partitions):
        ens[:, j] = clustering_function(
            instances, np.random.choice(ensemble_k_range))
    return ens
...
def random_feature_selection_ensemble(
        instances: ndarray, features_per_partition: int,
        ensemble_k: int, nr_partitions: int = 20,
        clustering_function: Callable = single_kmeans):
    """
    ...
    for j in range(nr_partitions):
        selected_dimensions = random.sample(
            dimension_indices, features_per_partition)
        ens[:, j] = clustering_function(
            instances[:, selected_dimensions], np.random.choice(ensemble_k))
    return ens
