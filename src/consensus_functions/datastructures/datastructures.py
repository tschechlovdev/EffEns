...
def membership_matrix(cluster_run):
    ...
    if reduce(operator.mul, cluster_run.shape, 1) != max(cluster_run.shape):
        ...
        cluster_ids = np.unique(np.compress(
            np.isfinite(cluster_run), cluster_run))
        ...
        return sparse.csr_matrix((data, indices, indptr), shape=(cluster_ids.size, cluster_run.size)).T
...
def binary_association_matrix(partition_label_vectors):
    ...
    for i in range(1, H):
        ba = sparse.hstack([ba, membership_matrix(
            partition_label_vectors[i])], format='csr')
    return ba
...
def cluster_coassociation_matrix(partition_pair):
    ...
    """
    return membership_matrix(partition_pair[:, 0]).T.dot(membership_matrix(partition_pair[:, 1]))
